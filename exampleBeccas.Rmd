...
#need to put output into separate data frames if using with sequence (below)

allsteps <- grow_tidy_expand_totals(1e-6, 1e-6, 0.005, 5)
...
ode_tol_values <- seq(1e-7, 1e-4, by=10) %>% map(~grow_tidy_expand_totals(oderel = .x))
...
sch_eps_values <- seq(0.001, 0.007, by=0.002) %>% map(~grow_tidy_expand_totals(scheps = .x))
