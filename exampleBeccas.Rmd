...
```{r, eval=FALSE}
remotes::install_github("traitecoevo/plant@ff16r", force = TRUE)
```
...
1. ode_tol_rel and ode_tol_abs set @ 1*10^-0.6
2. schedule_eps set @ 0.001 
...
-- Control parameters that tune various aspects of the numerical solvers --> Control(..., values = list(...))
...
```{r}
#base parameters, default arg values set as original control parameter values and no recr. decay
...
seed_rain_decay <- function(seedrain = 100, decay = 0, time){
  time = seq(0, 100, by=1)
  seedrain*exp(-decay*time)
}

seed_rain_decay(1000,5) %>% as_tibble() %>% 
  rename("seeds" = value) %>% mutate("year" = seq(0, 100, by=1)) %>%
  ggplot(aes(year, seeds)) +
  geom_line()
...
##create patch
results_decay6 <- 
  run_mypatch_edit(p0 = base_params(1e-6, 1e-6, 0.001, 0)) 
...
grow_tidy_expand_totals <- function(oderel=1e-6, odeabs=1e-6, scheps=0.001, rec_decay=0) {
  p = base_params(oderel, odeabs, scheps, rec_decay)
  patch <- run_mypatch_edit(p0 = p)
  patch_tidy <- tidy_patch(patch)
  patch_expand <- expand_state(patch_tidy)
  patch_totals <- patch_species_total(patch_expand$species)
  
}

#need to put output into separate data frames

allsteps <- grow_tidy_expand_totals(1e-6, 1e-6, 0.005, 0)
...
## varied ODE tolerance rel/abs and schedule eps

```{r}
## vary ODE tolerance & plot LA vs time results

ode_tol_values <- seq(1e-7, 1e-4, by=10) %>% map_dfr(~grow_tidy_expand_totals(oderel = .x))
#why not whole sequence working like below?

ode_tol_values %>% ggplot()


## vary schedule eps & plot LA vs time results

sch_eps_values <- seq(0.001, 0.007, by=0.002) %>% map_dfr(~grow_tidy_expand_totals(scheps = .x))
#need to split the output into separate data frames not one long one

sch_eps_values %>% ggplot(aes(time, area_leaf)) + geom_line()

```


```{r}
#varying these isn't changing the error/smoothness around LA plot? Times for steps still the same?

ode_tol_4 <- grow_tidy_expand_totals(1e-4, 1e-4)
ode_tol_5 <- grow_tidy_expand_totals(1e-5, 1e-5)
ode_tol_6 <- grow_tidy_expand_totals(1e-6, 1e-6)
ode_tol_7 <- grow_tidy_expand_totals(1e-7, 1e-7)

ode_tol_7 %>% ggplot(aes(time, area_leaf)) + geom_line()

sch_eps_1 <- grow_tidy_expand_totals(scheps=1e-1)
sch_eps_3 <- grow_tidy_expand_totals(scheps=1e-3)
sch_eps_5 <- grow_tidy_expand_totals(scheps=1e-5)
sch_eps_7 <- grow_tidy_expand_totals(scheps=1e-7)

sch_eps_7 %>% ggplot(aes(time, area_leaf)) + geom_line()
```




## Self-thinning -- background 
