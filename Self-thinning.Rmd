...
# 2. with mutate + map:
data_amax <- expand_grid(narea = seq(0.00187, 0.0031, by=0.0001), B_lf5 = c(1, 2)) %>%
  mutate(
    amax=map2_dbl(narea, B_lf5, ~amax_tradeoffs(narea = .x, B_lf5 = .y))
    )

ggplot(data_amax, aes(narea, amax, col=as.character(B_lf5))) + 
  geom_line() +
  theme_classic() +
  xlab("Narea [kg/m^2]") +
  ylab("Amax")
...
# add respiration function r_l  <- B_lf4 * narea / lma
# 1. with map2/pmap?
respiration <- function(B_lf4, narea, lma=0.07){
  B_lf4 * narea / lma
}

data_amax <- expand_grid(narea = seq(0.00187, 0.0031, by=0.0001), B_lf5 = c(1, 2), B_lf4 = c(19000, 21000, 23000)) %>% rowwise() %>%
  mutate(
    amax=map2_dbl(narea, B_lf5, ~amax_tradeoffs(narea = .x, B_lf5 = .y)),
    resp=map2_dbl(narea, B_lf4, ~respiration(narea = .x, B_lf4 = .y))
  )

ggplot(data_amax, aes(narea, resp, col=as.character(B_lf4))) + 
  geom_line() +
  theme_classic() +
  xlab("Narea [kg/m^2]") +
...
# 2. with mutate
data_amax <- expand_grid(narea = seq(0.00187, 0.0031, by=0.0001), B_lf5 = c(1, 2), B_lf4 = c(20000, 21000, 22000)) %>%
  mutate(
    amax=amax_tradeoffs(narea = narea, B_lf5=B_lf5),
    resp=respiration(narea = narea, B_lf4 = B_lf4)
    )
ggplot(data_amax, aes(narea, amax, col=as.character(B_lf5))) + 
  geom_line() +
  theme_classic() +
...
B_dI1 * (rho / rho_0) ^ (-B_dI2)

}

expand_grid(rho=seq(400, 800, by=10), B_dI2=c(0.5,3))%>% rowwise() %>% 
  mutate(
  DI= DI_tradeoffs(rho=rho, B_dI2=B_dI2)
) %>%
  ggplot(aes(rho, DI, col=as.character(B_dI2))) + 
  geom_line() +
  theme_classic() +
...
B_ks1 * (rho / rho_0) ^ (-B_ks2)

}

expand_grid(rho=seq(400, 800, by=10), B_ks2=c(0.5,3)) %>% rowwise() %>%
  mutate(
   KS=KS_tradeoffs(rho=rho, B_ks2=B_ks2) 
  ) %>%
ggplot(aes(rho, KS, col=as.character(B_ks2))) + 
  geom_line() +
  theme_classic() +
  xlab("wood density [kg/m^3]") +
