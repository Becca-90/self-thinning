...
height_plots <- ggplot() +
  ...
  geom_line(data = example_stand_nodes, aes(height, value, group = node), col = "green", size = 0.1) +
...
stem_diameter_plots <- ggplot() +
  ...
  geom_line(data = example_stand_nodes, aes(diameter_stem, value, group = node), col = "green", size = 0.1) +
  facet_wrap(~label, scales = "free") +
  scale_x_log10(labels = scales::comma) +
  scale_y_log10() +
  theme_classic(base_size = 18)
...
p1 = ggplot() +
  ...
  geom_line(data = example_stand_nodes, aes(age, height, group = node), col = "green", size = 0.1) +
  scale_y_log10(labels = scales::comma) +
  xlim(0, 500) +
  theme_classic(base_size = 18)
p2 = ggplot() +
  geom_point(data = baad_data, aes(age, diameter_stem, group = node), col = "grey", size = 0.5) +
  geom_line(data = example_stand_nodes, aes(age, diameter_stem, group = node), col = "green", size = 0.1) +
  scale_y_log10(labels = scales::comma) +
  xlim(0, 500) +
  theme_classic(base_size = 18)
age_plots = p1 + p2

ggsave(age_plots, filename = "../self-thinning/age_plots.pdf", width = 12, height = 5)
ggsave(stem_diameter_plots, filename = "../self-thinning/stem_diameter_plots.pdf", width = 14, height = 10)
ggsave(height_plots, filename = "../self-thinning/height_plots.pdf", width = 12, height = 10)
```