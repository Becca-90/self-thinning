...
b. Trapazoidal Rule in R → The Trapezoidal Rule is one of Closed Newton-Cotes formulas for approximating the definite integral of a function. We integrate by calculating and summing the areas of many consecutive trapezoids below our size-density curve, which gives a good approximation of the population dynamics in this case.
...
## some fitted data from Trouve et al
```{r}
fitted_trouve <-
  tibble(
...
Trait Values:
N_AREA = 0.00187 kg/m^2
WD = 700 kg/m^3
LMA = 0.07, 0.24, 0.5 kg/m^2
HMAT = 15m
```{r}
lma_runs <- 
    c(0.07, 0.24, 0.5) %>% 
...
lma_totals %>%
  ...
  ggplot(aes(diameter_stem_av*100, density*10000, col=run, label=my_label)) + 
  geom_line() + 
...
lma_totals %>%
  ...
  ggplot(aes(diameter_stem_av, density, group=species)) +
  geom_line() + 
...
lma_expand %>% drop_na() %>% filter(time > 0.2) %>% ggplot(aes(time, mortality, col = run)) + geom_line() + theme_classic()

## mortality, number of individuals that die(?) need to get yearly mortality rates?
```
...
Trait Values:
N_AREA = 0.00187, 0.003, 0.005 kg/m^2
WD = 700 kg/m^3
LMA = 0.07 kg/m^2
HMAT = 15m
```{r}
...
hmat_total %>%
  ...
  ggplot(aes(area_stem_av, density, col=run)) + 
  geom_line() +
  geom_text_repel(aes(label=my_label), nudge_x = 2) +
  scale_x_log10() +
  scale_y_log10() +
...
seed_mass_lines = seed_mass_totals %>%
filter(time > 0.2) %>%
  mutate(
    my_label = find_closest(round(time, 2), at = c(1.25, 2.5, 5, 10, 20, 40, 80))   
    ) %>%
...
```{r}
lma_lines + narea_lines + wd_lines + seed_mass_lines
```


```{r}
lma_lines = lma_values %>%
  filter(time > 0.2) %>%
  mutate(
      my_label = find_closest(round(time, 2), at = c(1.25, 2.5, 5, 10, 20, 40, 80))     
      ) %>%
...
filter(time > 0.2) %>%
  ...
  ggplot(aes(diameter_stem_av, density, col=run)) + 
  geom_line() +
  geom_abline(slope = -1.52, intercept = log(12.07), colour = "dark grey") +
  geom_abline(slope = -1.42, intercept = log(11.29), colour = "dark grey") +
  geom_abline(slope = -1.56, intercept = log(12.21), colour = "dark grey") +
  geom_abline(slope = -1.57, intercept = log(12.26), colour = "dark grey") +
  geom_text_repel(aes(label=my_label), nudge_x = 2) +
  scale_x_log10() +
  scale_y_log10() +
  theme_classic() +
...
ggplot(aes(diameter_stem_av, density, col = run)) +
  geom_line(data = wd_total) +
  geom_line(data = lma_values) +
  geom_line(data = narea_total)
...
- 3. Vary environmental params
- 4. Quantify slope, intercept, mortality rate, thinning period


```{r}
lma_values %>%
  filter(time > 0.2) %>%
  ggplot(aes(diameter_stem_av, log10(density), col=run)) + 
  geom_line() +
  theme_classic()

lma_values %>%
  filter(time > 0.2) %>%
  ggplot(aes(time, log10(density), col=run)) + 
  geom_line() +
  theme_classic()

lma_values %>%
  filter(time > 0.2) %>%
  ggplot(aes(time, diameter_stem_av, col=run)) + 
  geom_line() +
  theme_classic()
