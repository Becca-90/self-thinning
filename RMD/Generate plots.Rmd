...
#ggsave(intro_plot, filename = "../self-thinning/plots/intro_plot2.pdf", width = 12, height = 10)
```
...
#library(austraits) 
austraits_full <- load_austraits(version = "3.0.2", path = "data/austraits")
austraits <- austraits_full$traits

#austraits <- read.csv("data/traits.csv", header = TRUE)
woodytraits <- read.csv("data/woodiness_2022.03.19.csv", header = TRUE) %>% select(taxon_name, woodiness_recoded) 
austraits <- woodytraits %>% right_join(austraits, relationship = "many-to-many") %>% mutate(ID = row_number())

#austraits %>% filter(taxon_name == "Acanthus ebracteatus", woodiness_recoded == "woody") %>% View()
#austraits[160490,] %>% View()
#woodytraits %>% filter(taxon_name == "Suaeda maritima")
```
...
examplestand2 <- grow_tidy_expand_totals(a_dG2 = 30, B_kl2 = 1.73)
stand_totals2 <- examplestand2$patch_total  %>%
  mutate(
    study = "plant",
    replicate = 1,
    #diameter_stem_av = (diameter_stem_av*100), #convert m to cm
